  ...
  const signUpWithGoogle = async () => {
    try {
      ...
      const member = members.find(
        (item) => item["Institute Email Id"] === email
      );

      if (member) {
        const emailFound = await verifyEmail(email);
        console.log(emailFound);

        if (emailFound) {
          toast.error("You have already signed up! Please log in.", {
            duration: 5000,
          });
          router.push("/login");
        } else {
          const newMember = {
            name: member["Name"],
            rollNo: member["Roll Number"],
            email: member["Institute Email Id"],
          };
          console.log(newMember);

          try {
            await membersDataService.addMembers(newMember);
          ...
          <div className="flex justify-center items-center h-[100vh] overflow-hidden bg-inherit">
            <div className="bg-white shadow-md rounded-lg p-8 w-full max-w-sm flex-shrink-0">
              ...
              <div className="text-center pb-4">
                <h2 className="text-4xl font-bold mb-1  text-sky-600">
                  S<span className="text-black">ochem</span>
                </h2>
              </div>
